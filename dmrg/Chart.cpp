...
std::list<CString> Chart::GetXLabels() const
{
	...
	double interval = abs (xmax - xmin) / ticks;

	for (int i = (drawStartTickX ? 0 : 1); i <= ticks; ++i)
	{
		CString str;
		str.Format(L"%.2f", static_cast<double>(xmin) + interval*i);
		l.push_back(str);
	}
...
std::list<CString> Chart::GetYLabels() const
{
	...
	const double interval = abs (ymax - ymin) / ticks;


	for (int i = (drawStartTickY ? 0 : 1); i <= ticks; ++i)
	{
		CString str;
		str.Format(L"%.2f", static_cast<double>(ymin) + interval*i);
		l.push_back(str);
	}
